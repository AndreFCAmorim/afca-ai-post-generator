...
class AIPG_Cron {
    ...
    private function __construct() {
        ...
        add_action( AIPG_CRON_HOOK, [ $this, 'run_generation' ] );
    }
    ...
    public static function schedule(): void {
        if ( ! wp_next_scheduled( AIPG_CRON_HOOK ) ) {
            $recurrence = AIPG_Settings::get_schedule();
            wp_schedule_event( time(), $recurrence, AIPG_CRON_HOOK );
        }
    }
    ...
    public static function unschedule(): void {
        $timestamp = wp_next_scheduled( AIPG_CRON_HOOK );
        if ( $timestamp ) {
            wp_unschedule_event( $timestamp, AIPG_CRON_HOOK );
        }
        // Also clear all future hooks in case of multiple entries
        wp_clear_scheduled_hook( AIPG_CRON_HOOK );
    }
    ...
    public static function next_run() {
        return wp_next_scheduled( AIPG_CRON_HOOK );
    }
    ...
    public function run_generation(): void {
        ...
        update_option( AIPG_LOG_OPTION, $log );
    }
}
